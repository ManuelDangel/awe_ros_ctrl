...
void FwNMPC::aslctrlDataCb(const mavros_msgs::AslCtrlData::ConstPtr& msg) {
  ...
  if (FAKE_SIGNALS == 1) {
    ...
    subs_.aslctrl_data.rollAngle = 0.0
        + 0.05 * (((double) rand() / (RAND_MAX)) * 2.0 - 1.0);
    subs_.aslctrl_data.pitchAngle = y_theta_ref
        + 0.1 * fabs(y_theta_ref)
            * (((double) rand() / (RAND_MAX)) * 2.0 - 1.0);
    subs_.aslctrl_data.p = 0.0;
    subs_.aslctrl_data.q = 0.0;
    subs_.aslctrl_data.r = 0.0;
    subs_.aslctrl_data.airspeedRef = 13.5
        + 0.2 * fabs(13.5) * (((double) rand() / (RAND_MAX)) * 2.0 - 1.0);
    subs_.aslctrl_data.uThrot = y_uT_ref + CTRL_DEADZONE[0];
  } else {
  ...
  const double vn_minus_wn = subs_.odom.twist.twist.linear.x
      - (double) subs_.ekf_ext.windSpeed
          * cos((double) subs_.ekf_ext.windDirection);
  const double ve_minus_we = subs_.odom.twist.twist.linear.y
      - (double) subs_.ekf_ext.windSpeed
          * sin((double) subs_.ekf_ext.windDirection);
  ...
  if (delta_yaw < -3.141592653589793)
    delta_yaw = delta_yaw + 6.283185307179586;
  if (delta_yaw > 3.141592653589793)
    delta_yaw = delta_yaw - 6.283185307179586;
...
void FwNMPC::odomCb(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if (FAKE_SIGNALS == 1 || FAKE_SIGNALS == 2) {
    subs_.odom.twist.twist.linear.x = 13.5 * cos(subs_.aslctrl_data.yawAngle)
        + 4.0 * cos(1.047);  //vn
    subs_.odom.twist.twist.linear.y = 13.5 * sin(subs_.aslctrl_data.yawAngle)
        + 4.0 * sin(1.047);  //ve
...
void FwNMPC::ekfExtCb(const mavros_msgs::AslEkfExt::ConstPtr& msg) {
  ...
  if (FAKE_SIGNALS == 1) {
    ...
    subs_.ekf_ext.airspeed = 13.5;
    subs_.ekf_ext.windSpeed = 4.0
        + 0.2 * 4.0 * (((double) rand() / (RAND_MAX)) * 2.0 - 1.0);
    subs_.ekf_ext.windDirection = 1.047
        + 0.3 * (((double) rand() / (RAND_MAX)) * 2.0 - 1.0);
    subs_.ekf_ext.windZ = -1.0;
  } else {
...
void FwNMPC::initHorizon() {
  ...
  ROS_ERROR(
      "initHorizon: hold states constant through horizon (first time in loop)");
...
//	}
  if (fabs(U[0]) < CTRL_DEADZONE[0])
    U[0] = 0.0;  // in dead-zone : set to zero
  else if (U[0] < 0.0)
    U[0] = U[0] + CTRL_DEADZONE[0];  // negative control value : add dead-zone threshold
  else
    U[0] = U[0] - CTRL_DEADZONE[0];  // positive control value : subtract dead-zone threshold

  // normalize for internal model //
  for (int i = 0; i < NU; ++i)
    U[i] = U[i] / CTRL_NORMALIZATION[i];

  // saturate controls for internal model constraints //
  for (int i = 0; i < NU; ++i) {
    if (U[i] < CTRL_SATURATION[i][0])
      U[i] = CTRL_SATURATION[i][0];
    if (U[i] > CTRL_SATURATION[i][1])
      U[i] = CTRL_SATURATION[i][1];
  }
  ...
  for (int i = 0; i < N + 1; ++i) {
    for (int j = 0; j < NX - NX_AUGM; ++j) {
      if (j == 0) {
        acadoVariables.x[i * NX + j] = X0[0]
            + subs_.odom.twist.twist.linear.x * getTimeStep() * (double) i;  // linear propagation
      } else if (j == 1) {
        acadoVariables.x[i * NX + j] = X0[1]
            + subs_.odom.twist.twist.linear.y * getTimeStep() * (double) i;  // linear propagation
      } else if (j == 2) {
        acadoVariables.x[i * NX + j] = X0[2]
            + subs_.odom.twist.twist.linear.z * getTimeStep() * (double) i;  // linear propagation
...
void FwNMPC::updateACADO_X0() {
  ...
  for (int i = 0; i < NX; ++i)
    temp_in[i] = acadoVariables.x0[i];
  for (int i = NX; i < NX + NOD; ++i)
    temp_in[i] = acadoVariables.od[i - NX];
...
void FwNMPC::updateACADO_OD() {
  ...
  OD[16] = (double) subs_.ekf_ext.windSpeed
      * cos((double) subs_.ekf_ext.windDirection);
  OD[17] = (double) subs_.ekf_ext.windSpeed
      * sin((double) subs_.ekf_ext.windDirection);
...
void FwNMPC::reqSubs() {  //TODO: extend this and/or change this to all subs/initializations
  ...
  ros::ServiceClient wp_pull_client_ = nmpc_
      .serviceClient<mavros_msgs::WaypointPull>("/mavros/mission/pull");
  mavros_msgs::WaypointPull wp_pull_srv;

  if (wp_pull_client_.call(wp_pull_srv)) {
    ROS_ERROR("reqSubs: received %d waypoints",
              (int )wp_pull_srv.response.wp_received);
  } else {
...
void FwNMPC::calculateTrackError(const real_t *in) {
  ...
  if (pparam_type < 0.5) {
    ...
    // calculate tangent
    tP_n = cos(in[idx_OD_0 + pparam_sel + 6])
        * cos(in[idx_OD_0 + pparam_sel + 5]);
    tP_e = cos(in[idx_OD_0 + pparam_sel + 6])
        * sin(in[idx_OD_0 + pparam_sel + 5]);
    tP_d = -sin(in[idx_OD_0 + pparam_sel + 6]);

    // dot product
    const double dot_tP_bp = tP_n * (in[0] - in[idx_OD_0 + pparam_sel + 1])
        + tP_e * (in[1] - in[idx_OD_0 + pparam_sel + 2])
        + tP_d * (in[2] - in[idx_OD_0 + pparam_sel + 3]);
    ...
    const double pparam_ldir =
        (in[idx_OD_0 + pparam_sel + 4] < 0.0) ? -1.0 : 1.0;
    ...
    p_n = fabs(in[idx_OD_0 + pparam_sel + 4]) * cr_n_unit
        + in[idx_OD_0 + pparam_sel + 1];
    p_e = fabs(in[idx_OD_0 + pparam_sel + 4]) * cr_e_unit
        + in[idx_OD_0 + pparam_sel + 2];
    ...
    // angular exit
    double xi_exit = in[idx_OD_0 + pparam_sel + 5]
        - pparam_ldir * 1.570796326794897;
    ...
    if (fabs(in[idx_OD_0 + pparam_sel + 6]) < 0.001) {
      ...
      const double RtanGam = fabs(in[idx_OD_0 + pparam_sel + 4])
          * tan(in[idx_OD_0 + pparam_sel + 6]);

      // height down from exit
      const double delta_d_xi = delta_xi * RtanGam;

      // nearest spiral leg
      const double delta_d_k = round(
          (in[2] - (in[idx_OD_0 + pparam_sel + 3] + delta_d_xi))
              / (6.28318530718 * RtanGam)) * 6.28318530718 * RtanGam;

      // closest point on nearest spiral leg
      p_d = in[idx_OD_0 + pparam_sel + 3] + delta_d_k + delta_d_xi;

      // cap end point
      if ((p_d - in[idx_OD_0 + pparam_sel + 3]) * in[idx_OD_0 + pparam_sel + 6]
          < 0.0) {
        p_d = in[idx_OD_0 + pparam_sel + 3];
        tP_d = 0.0;
        Gam_temp = 0.0;
  ...
  const double too_close = fabs(
      track_error_lat_
          / ((norm_vG_lat < 1.0) ?
              1.0 :
              (((double) subs_.nmpc_params.T_b_lat) * norm_vG_lat
                  + norm_vG_lat * norm_vG_lat / 5.6638)));  //XXX: this is just a hack to keep zero vG safe for the time being - need to incorporate with later smooth boundary limiter
  double sig_e = 0.0;
  if (too_close < 1.0) {
  ...
  if (dot_tP_vG < 0.0)
    sig_eta = dot_tP_vG * dot_tP_vG
        / ((norm_vG_lat < 1.0) ? 1.0 : norm_vG_lat * norm_vG_lat);  //XXX: this is just a hack to keep zero vG safe for the time being - need to incorporate with later smooth boundary limiter

  // variable (externally input) T_b_lat
  T_b_lat_ = ((double) subs_.nmpc_params.T_b_lat)
      * (1.0 + 8.0 * sig_eta * sig_e);
...
void FwNMPC::publishControls(uint64_t &t_ctrl, uint64_t t_iter_approx,
                             int obctrl_status) {
  ...
  // de-normalize //
  for (int i = 0; i < NU; ++i)
    ctrl[i] = ctrl[i] * CTRL_NORMALIZATION[i];
...
//		else ctrl[ i ] = 0.0;
//	}
  if (ctrl[0] < 0.02 * CTRL_NORMALIZATION[0] * CTRL_SATURATION[0][0])
    ctrl[0] = ctrl[0] - CTRL_DEADZONE[0];
  else if (ctrl[0] > 0.02 * CTRL_NORMALIZATION[0] * CTRL_SATURATION[0][1])
    ctrl[0] = ctrl[0] + CTRL_DEADZONE[0];
  else
    ctrl[0] = 0.0;
...
void FwNMPC::publishNmpcInfo(ros::Time t_iter_start, uint64_t t_ctrl,
                             uint64_t t_solve, uint64_t t_update,
                             uint64_t t_wp_man) {
